...
private updateGame() {
  // 0) Tant que la balle n’est pas active OU que la partie est déjà terminée, on ne fait rien
  if (!this.isBallActive || this.isGameOver) {
    return;
  }

  // 1) Déplacement vertical des paddles
  ...
  if (this.ball.x - halfSize < 0) {
    // « Player 2 » vient de marquer un point
    this.score.player2++;
    this.updateScoreDisplay();

    if (this.score.player2 >= this.settings.endScore) {
      // Le match est terminé → on appelle endGame, et endGame enverra la requête
      this.endGame('Player 2');
    } else {
      // Le match continue, on replace la balle pour le point suivant
      this.resetBall();
    }
  ...
  if (this.ball.x + halfSize > this.canvas.width) {
    // « Player 1 » vient de marquer un point
    this.score.player1++;
    this.updateScoreDisplay();
  ...
  // Sinon, aucun but, la partie continue…
}
